    ...
    actions: {
      ...
      updateZoom({ context, event, prop }) {
        ...
        if (typeof targetZoom === "number") {
          nextZoom = utils.clampValue(targetZoom, minZoom, maxZoom);
          ...
          nextZoom = utils.clampValue(currentZoom * smoothing, minZoom, maxZoom);
        } else if (typeof delta === "number") {
          const direction = Math.sign(delta) < 0 ? 1 : -1;
          nextZoom = utils.clampValue(currentZoom + step * direction, minZoom, maxZoom);
        } else {
          return;
        }
        if (nextZoom === currentZoom && panDelta) {
          const currentOffset2 = context.get("offset");
          const rotation = context.get("rotation");
          const nextOffset2 = clampOffset({
            zoom: currentZoom,
            rotation,
            viewportSize: viewportRect,
            offset: addPoints(currentOffset2, panDelta),
            fixedCropArea: prop("fixedCropArea"),
            crop: context.get("crop"),
            naturalSize: context.get("naturalSize")
          });
